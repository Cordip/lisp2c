...
object Lowering:
  ...
  private val primitiveMap: Map[String, String] = Map(
    "cons" -> makeCons,
    "+" -> lispAdd,
    "-" -> lispSub,
    "*" -> lispMul,
  ...
  private def lowerExprWithName(expr: LispExpr, hint: String, scope: Scope): CExpr =
    expr match
      case LispLambda(params, body, freeVars) =>
        val funcName = s"lisp_${hint.replaceAll("[^a-zA-Z0-9_]", "_")}_$counter"
        counter += 1
  ...
  def lowerExpr(expr: LispExpr, scope: Scope): CExpr =
    expr match
      ...
      case LispIf(cond, thenBranch, elseBranch) => CIf(lowerExpr(cond, scope), lowerExpr(thenBranch, scope), lowerExpr(elseBranch, scope))
      case LispVar(name) if primitiveMap.contains(name) => throw new Exception(s"Primitive $name used as value — not supported yet")
      case LispVar(name) => scope.resolve(name)
      case LispLambda(params, body, freeVars) => lowerLambda(s"lambda_$counter", params, body, freeVars, scope)
      case LispApply(fn, _) if isLiteral(fn) => throw new Exception(s"${fn.show} is not callable")
      case LispApply(LispVar(name), args) if primitiveMap.contains(name) => CCall(primitiveMap(name), args.map(lowerExpr(_, scope)))
      case LispApply(fn, args) => CApplyClosure(lowerExpr(fn, scope), args.map(lowerExpr(_, scope)))
      case LispLet(bindings, body) =>
        val bindingExprs = bindings.map((_, v) => lowerExpr(v, scope))
        val bindingNames = bindings.map(_._1)
        val innerScope = Scope(envVars = bindingNames, globals = scope.globals, parent = Some(scope))
        val bodyExpr = lowerExpr(body, innerScope)
        CLet(bindingExprs, bodyExpr)
      case _ => throw new Exception(s"Lowering: unsupported: $expr")

  private def lowerLambda(funcName: String, params: List[String], body: LispExpr, freeVars: List[String], scope: Scope): CExpr =
    counter += 1
    val innerScope = Scope(params = params, envVars = freeVars, globals = scope.globals)
    functions += CFunction(funcName, params, lowerExpr(body, innerScope))
