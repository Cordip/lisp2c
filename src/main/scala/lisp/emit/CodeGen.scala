...
import lisp.types.{CExpr, FlatFunction, GlobalDecl, Line, Statement}
...
object CodeGen:
  ...
  private def emitStatement(stmt: Statement): List[Line] =
    stmt match
      ...
      case EnvSet(envName, index, valueName) =>
        List(Text(s"$envName->vars[$index] = $valueName;"))
      case PrintVal(varName) =>
        List(Text(s"print_val($varName);"), Text(s"""printf("\\n");"""))
